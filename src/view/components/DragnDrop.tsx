...
export default function DragnDrop(props : {onDragging?: (event : MouseEvent, draggingParameters: DraggingParameters[]) => void, onDraggingEnd?: (event : MouseEvent, draggingParameters: DraggingParameters[]) => void}) {
  const dragging = useViewModelStore((state) => state.dragging);
  const draggingParameters = useViewModelStore((state) => state.draggingParameters);
  const stopDragging = useViewModelStore((state) => state.stopDragging);
  ...
  const setElementPos = (draggingParameters : DraggingParameters, x: number, y: number, reset: boolean) => {
    ...
    if (element) {
      ...
      if (reset && draggingParameters.initialStyle) {
        element.style.zIndex = draggingParameters.initialStyle.zIndex;
        element.style.pointerEvents =  '';
        element.style.position = draggingParameters.initialStyle.position;
        element.style.opacity = draggingParameters.initialStyle.opacity;
        //element.style.left = draggingParameters.initialStyle.left;
        //element.style.top = draggingParameters.initialStyle.top;
      } else {
        element.style.zIndex = reset ? '' : '1000';
        element.style.pointerEvents =  reset ? '' : 'none';
        element.style.opacity =  reset ? '' : '0.3';
    ...
    const onMouseUp = (e: MouseEvent) => {
      if (dragging) {
        for (const draggingParameter of draggingParameters) {
            if (draggingParameter.onElementDropped) {
                const element = getDraggedElement(draggingParameter, false);
                draggingParameter.onElementDropped(draggingParameter, element);
            }
            setElementPos(draggingParameter, e.clientX - draggingParameter.offsetX, e.clientY - draggingParameter.offsetY, true);
        }
        stopDragging();
        if (props.onDraggingEnd) {
          props.onDraggingEnd(e, draggingParameters);
        }
    ...
    const onMouseMove = (e: MouseEvent) => {
      if (dragging) {
        if (e.buttons === 0 && (e as any)['mozInputSource'] === undefined) {
          ...
          if (props.onDragging) {
            props.onDragging(e, draggingParameters);
          }
